...
当前发布 **0.0.1 测试版**。测试阶段按 `0.0.x` 递增，正式上线时再发布 `1.0.0`。早期试验中误用的 `1.0.0`、`1.0.1` 已从当前发布目录撤回，原文件可从 Git 历史恢复，不作为正式发布。

曾安装早期 `1.0.x` 试验包的测试环境会触发客户端的防降级检查。需要改用 `0.0.1` 时，停止脚本并关闭客户端，先备份用户脚本目录，再将 `BDSP/.rng-package.json` 移到该目录之外保存；保留脚本和图像标签，重新打开客户端，检查更新后预览安装 `0.0.1`，选择保留本地修改。该操作只重建包安装记录。
...
packages/bdsp-official/0.0.1.zip       可安装的完整测试包
