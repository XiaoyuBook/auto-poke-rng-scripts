...
function build(root) {
  ...
  for (const id of fs.readdirSync(path.join(root, 'bundles')).sort()) {
    ...
    if (manifest.schemaVersion !== 1 || manifest.id !== id || !version(manifest.version) || !version(manifest.minimumAppVersion) || !manifest.name || !manifest.description || !Array.isArray(manifest.authors) || !manifest.authors.length) throw Error('包描述无效');
    if (!safePath(manifest.installFolder) || manifest.installFolder.includes('/') || folders.has(manifest.installFolder.toLowerCase())) throw Error('安装目录无效或重复');
    ...
    function visit(relative = '') {
      for (const item of fs.readdirSync(path.join(directory, 'files', relative), { withFileTypes: true }).sort((a,b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0)) {
        ...
        if (bytes.length > 12 * 1024 * 1024) throw Error('文件过大');
        if (/\.il$/i.test(name)) {
